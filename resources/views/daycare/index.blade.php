...
@section('content')
    ...
    <section class="cards-grid grid gap-4 lg:grid-cols-2">
        <article class="card rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <ul class="schedule-list mt-3 list-disc space-y-2 pl-5 text-slate-600">
        ...
        <article class="card rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <form action="{{ route('daycare.store') }}" method="post" class="stack-form mt-4 grid gap-3">
                @csrf
                ...
                <button type="submit" class="btn inline-flex w-fit rounded-lg bg-emerald-700 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-800">Aanmelden</button>
            </form>
        </article>
    </section>
@endsection
